...
def exibir_lancamentos():
    ...
    # CORREÇÃO AQUI: Removido .do_not_minify()
        ...
        init(params) {
            ...
            if (editButton) {
                editButton.addEventListener('click', () => {
                    ...
                    // Usar Streamlit.setComponentValue para enviar dados de volta
                    // O valor enviado pode ser lido no retorno de AgGrid() no Python
    ...
    """)._js_code # Este ._js_code foi a causa de um erro anterior, mas foi reintroduzido acidentalmente na revisão. Vamos removê-lo novamente.

    # CORREÇÃO FINAL DA DEFINIÇÃO do renderizador: Removendo o ._js_code extra
    cell_renderer_jscode = JsCode("""
    class ActionButtonRenderer {
        init(params) {
            this.eGui = document.createElement('div');
    ...
    """) # Sem ._js_code ou .do_not_minify()


    go.configure_column(
        'Ações',
        cellRenderer=cell_renderer_jscode, # Usa o renderizador JS personalizado (o objeto JsCode)
    ...
    # Habilitar enable_async_events=True e um listener onCellClicked pode ser uma alternativa
    # para capturar eventos de clique mais diretamente.
    # go.on('cellClicked', JsCode("""
    # function(params) {
    #     // params.data contém os dados da linha
    #     // params.colDef.field contém o nome da coluna clicada
    #     // params.event.target contém o elemento clicado (útil para ver se foi o botão)
    #     if (params.colDef.field === 'Ações' && params.event.target.tagName === 'BUTTON') {
    #         const originalIndex = params.event.target.getAttribute('data-original-index');
    #         const action = params.event.target.classList.contains('btn-edit') ? 'edit' : 'delete';
    #         Streamlit.setComponentValue({ action: action, index: originalIndex });
    #     }
    # }
    # """)) # Habilita listener de clique na célula e envia dados via setComponentValue


    gridOptions = go.build()

    # --- Exibe a tabela AgGrid e captura eventos ---
    # enable_async_events=True é necessário para capturar eventos de clique na célula via .on() listener.
    # Com o JsCode no cellRenderer usando Streamlit.setComponentValue,
    # a informação do clique deve aparecer em grid_response['value'].
    grid_response = AgGrid(
        ...
        # fit_columns_on_grid_load=True, # Pode ser removido se preferir definir larguras manuais
        allow_unsafe_jscode=True, # **CRUCIAL** para o JavaScript no cellRenderer funcionar e para o JsCode listener se usado
        ...
        # enable_async_events=True # Habilita eventos assíncronos para listeners .on()
    )

    # --- Captura e processa eventos enviados pelo JavaScript dos botões ---
    # O Streamlit.setComponentValue no JsCode do cellRenderer envia dados que aparecem em grid_response['value'].
    # Vamos verificar essa chave para a nossa estrutura { action: ..., index: ... }.

    # Verifica se a resposta da AgGrid contém informações de ação (enviadas pelo JS no cellRenderer)
    if grid_response is not None and isinstance(grid_response, dict):
        # Verifica se a chave 'value' existe na resposta e se é um dicionário com 'action' e 'index'
        if 'value' in grid_response and isinstance(grid_response['value'], dict):
            action_data = grid_response['value']
            if 'action' in action_data and 'index' in action_data:
                 action = action_data['action']
                 # Converte o índice para int. O índice vem como string do JavaScript.
                 try:
                    index = int(action_data['index'])
                 except ValueError:
                    st.error("Erro ao processar índice do lançamento.")
                    return # Sai se o índice não for válido

                 # Verifica se o índice retornado é válido na lista original antes de prosseguir
                 if 0 <= index < len(st.session_state.get("lancamentos", [])):
                    if action == 'edit':
                        # Define o estado para solicitar a exibição do formulário de edição
                        st.session_state['edit_requested_index'] = index
                        st.rerun() # Rerun para ativar o modal de edição
                    elif action == 'delete':
                        # Define o estado para solicitar a confirmação de exclusão
                        st.session_state['awaiting_delete_confirmation_index'] = index
                        st.rerun() # Rerun para exibir a mensagem de confirmação
                 else:
                    st.error("Erro: Índice de lançamento retornado inválido.")
        # Nota: Esta lógica de captura depende de como Streamlit.setComponentValue
        # se manifesta no retorno da AgGrid em diferentes versões.
        # Se esta lógica de captura não funcionar, a abordagem com `enable_async_events=True`
        # e um listener `onCellClicked` seria a alternativa mais robusta.
