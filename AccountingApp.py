...
def exibir_lancamentos():
    ...
    # CORREÇÃO AQUI: Removido ._js_code
    ...
    """).do_not_minify() # Adicionado .do_not_minify() opcionalmente para debug, ou pode ser removido
    ...
    # para capturar eventos de clique mais diretamente, mas o JsCode no cellRenderer já tenta enviar
    # a informação via Streamlit.setComponentValue().

    # Vamos tentar capturar eventos de clique na célula para ter uma referência extra,
    # embora a lógica de botão no cellRenderer seja mais específica.
    ...
    #     }
    # }
    # """).do_not_minify()) # Habilita listener de clique na célula
    ...
    # enable_async_events=True é necessário para capturar eventos de clique na célula via .on()
    # Mas o JsCode no cellRenderer usando Streamlit.setComponentValue não precisa explicitamente dele.
    # Vamos manter allow_unsafe_jscode=True para o HTML e JsCode no cellRenderer.
    grid_response = AgGrid(
        ...
        # enable_async_events=True # Habilita eventos assíncronos se usar listeners .on()
    )

    # --- Captura e processa eventos enviados pelo JavaScript dos botões ---
    # A forma como o Streamlit.setComponentValue no JsCode interage com o Streamlit
    # pode fazer com que o valor enviado apareça em 'value' ou em outra parte da resposta.
    # Vamos verificar a resposta para a nossa estrutura { action: ..., index: ... }.

    # Verifica se a resposta da AgGrid contém informações de ação (enviadas pelo JS no cellRenderer)
    # A informação enviada pelo Streamlit.setComponentValue geralmente aparece em grid_response['value']
    if grid_response is not None and isinstance(grid_response, dict):
        ...
        if 'value' in grid_response and isinstance(grid_response['value'], dict):
            ...
            if 'action' in action_data and 'index' in action_data:
                 ...
                 index = int(action_data['index']) # O índice vem como string do JS
        ...
        # se manifesta no retorno da AgGrid em diferentes versões. Testes podem ser necessários.
        # Se não funcionar, o uso de onCellClicked com enable_async_events=True é a alternativa mais robusta.
